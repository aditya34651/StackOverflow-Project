...
def CreateUser(request: schemas.User, db: Session = Depends(get_db)):
    ...
    quer = db.query(models.User).filter(models.User.email==request.email).first()
    if not quer:
        db.add(new_user)
        db.commit()
        db.refresh(new_user)
...
@router.get('/user', response_model=List[schemas.ShowUsers], tags=['Users'])
def showusers(db:Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    Users = db.query(models.User).all()
    return Users   

@router.get('/user/{id}',  status_code=200, response_model=schemas.ShowUsers, tags=['Users'])   
